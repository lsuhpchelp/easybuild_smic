...
class EB_PETSc(ConfigureMake):
    ...
    def configure_step(self):
        ...
        if LooseVersion(self.version) >= LooseVersion("3"):
            ...
            #la_dir = os.getenv('LAPACK_LIB_DIR')
            #bl_dir = os.getenv('BLAS_LIB_DIR')
            #la_libs = os.getenv('LAPACK_LIB')
            #bl_libs = os.getenv('BLASS_LIB')
            #print la_dir, la_libs, bl_dir, bl_libs
            #if la_dir and bl_dir:
            #    self.cfg.update('configopts', '--with-lapack-lib=[%s/%s]' % (la_dir, la_libs))
            #    self.cfg.update('configopts', '--with-blas-lib=[%s/%s]' % (bl_dir, bl_libs))
